param([string]$Agent="local",[string]$Purpose="unspecified",[string[]]$Files=@(),[string]$Prompt="")
...
$msg = [ordered]@{
  ...
  purpose = $Purpose
  files = $Files
  payload = @{ model = "placeholder"; prompt = $Prompt; metadata = @{ created_by = $env:USERNAME } }
  status = "pending"
  approval = @{ required = $true; approved_by = $null; approved_at = $null }
}
$msgPath = Join-Path $Queue ("message-$id.json")
$msg | ConvertTo-Json -Depth 10 | Out-File -FilePath $msgPath -Encoding utf8 -Force
...
"$id,$($msg.created_at),$Agent,$Purpose,$msgPath,pending,," | Out-File -FilePath $auditCsv -Encoding utf8 -Append
